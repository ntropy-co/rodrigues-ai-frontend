...
export const stepValuesSchema = z.object({
  ...
  quantity: z.number({ invalid_type_error: 'Quantidade inválida' }).positive('Quantidade deve ser positiva'),
...
export const stepGuaranteesSchema = z.object({
  guaranteeType: z.array(z.string()).min(1, 'Selecione pelo menos uma garantia'),
  ...
  if (data.hasGuarantor) {
    return !!data.guarantorName && !!data.guarantorCpfCnpj && !!data.guarantorAddress
  }
  return true
}, {
