...
function SimpleLineChart({
  ...
  const activeY =
    activePoint ? height - ((activePoint.close - domainMin) / domainRange) * height : 0
  ...
  return (
    <div ref={containerRef} className="relative select-none" style={{ height }}>
        ...
        {[0, 0.5, 1].map((p) => {
           ...
           return (
             <g key={p}>
               ...
               <text x="-5" y={y + 4} textAnchor="end" fontSize="10" fill="#6b7280" className='hidden sm:block'>
                 {price.toFixed(2)}
               </text>
             </g>
           )
        ...
        {cprY >= 0 && cprY <= height && (
          <g>
            <line
              ...
              strokeDasharray="5 5"
            />
            <text x="5" y={cprY - 5} fontSize="10" fill="#ef4444" fontWeight="bold">
      ...
      {activePoint && (
        <div
          className="pointer-events-none absolute rounded-md border bg-popover px-2 py-1 text-xs shadow-md text-popover-foreground"
          style={{
             left: activeX,
...
export function QuotesChart() {
  ...
  const commodity = COMMODITY_INFO[symbol] || { name: 'Desconhecido', unit: '-' }

  // Query
  const { data, isLoading, isError, refetch } = useQuery<QuotesHistoryResponse>({
    queryKey: ['quotes-history', symbol, range],
    queryFn: async () => {
      const res = await fetch(`/api/quotes/history?symbol=${symbol}&range=${range}`)
  ...
  const previousPrice = hasData && history.length > 1 ? history[history.length - 2].close : lastPrice

  const prices = history.map(h => h.close)
  const minPrice = hasData ? Math.min(...prices) : 0
  const maxPrice = hasData ? Math.max(...prices) : 0
  const avgPrice = hasData ? prices.reduce((a, b) => a + b, 0) / prices.length : 0
  ...
  const cprDiffPercent = cprPrice && lastPrice ? (cprDiff! / cprPrice) * 100 : null
    ...
    <Card className="w-full">
      <CardHeader className="flex flex-col space-y-4 sm:flex-row sm:items-start sm:justify-between sm:space-y-0">
        ...
        <div className="flex flex-wrap gap-2">
           <Select value={symbol} onValueChange={(v) => setSymbol(v as CommoditySymbol)}>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
           {isLoading ? (
             ...
             <>
               ...
               />
               <StatBox label="Média do Período" value={formatCurrency(avgPrice)} />
        ...
        <div className="min-h-[250px] w-full rounded-lg border bg-card p-4">
           {isLoading ? (
             ...
             <div className="flex h-[200px] flex-col items-center justify-center gap-2 text-destructive">
               ...
               <button onClick={() => refetch()} className="flex items-center gap-1 text-xs underline">
        ...
        <div className="flex flex-col gap-4 rounded-md bg-secondary/20 p-4 sm:flex-row sm:items-center">
           ...
           {cprPrice && lastPrice > 0 && (
             <div className="flex flex-1 items-center gap-4 rounded-md border bg-background p-3 shadow-sm">
                <div>
                  <p className="text-xs text-muted-foreground">Diferença (Mercado - CPR)</p>
                  <div className={cn("text-lg font-bold", isCprBetter ? "text-green-600" : "text-red-600")}>
                    {cprDiff && cprDiff > 0 ? '+' : ''}{formatCurrency(cprDiff || 0)}
                  </div>
                </div>
                <div className="text-sm">
                   {isCprBetter ? (
                      <span className="text-green-700">Mercado está pagando <b>{cprDiffPercent?.toFixed(1)}%</b> acima da sua CPR.</span>
                   ) : (
                      <span className="text-red-700">Mercado está <b>{Math.abs(cprDiffPercent || 0).toFixed(1)}%</b> abaixo da sua CPR.</span>
                   )}
                </div>
             </div>
           )}
...
function StatBox({ label, value, subValue, subColor, icon: Icon }: any) {
  return (
    <div className="rounded-md border p-3">
      ...
      <div className="mt-1 flex items-end gap-2">
        ...
        {subValue && (
            <div className={cn("flex items-center text-xs font-medium", subColor)}>
