...
export function ChatArea({ messages, isStreaming }: ChatAreaProps) {
  ...
  const { showScrollButton, scrollToBottom } = useScrollDetection({ containerRef })
    ...
    <div ref={containerRef} className="relative flex-1 overflow-y-auto px-4 py-6 md:px-6">
            ...
            >
              <div
                className={`group relative max-w-[85%] rounded-2xl px-4 py-3 ${
                  isUser ? 'bg-gemini-blue text-white' : 'bg-muted text-foreground'
                }`}
              >
                  ...
                  <div className="flex items-center gap-2">
                    <div
                      className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-medium ${
                        isUser ? 'bg-white/20 text-white' : 'bg-gemini-blue text-white'
                      }`}
                    >
                      {isUser ? 'U' : 'AI'}
                    </div>
      ...
      {showScrollButton && (
        <button
          ...
          className="fixed bottom-24 right-6 z-10 flex h-11 w-11 items-center justify-center rounded-full bg-gemini-blue text-white shadow-lg transition-all hover:bg-gemini-blue-hover hover:shadow-xl active:scale-95 md:bottom-28 md:right-8"
          style={{ minHeight: `${MIN_TOUCH_TARGET_SIZE}px`, minWidth: `${MIN_TOUCH_TARGET_SIZE}px` }}
