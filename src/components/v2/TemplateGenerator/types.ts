...
export type DocumentTypeId = 'cpr-fisica' | 'cpr-financeira' | 'contrato-compra-venda'
...
export interface DocumentField {
  name: string
  label: string
  type: 'text' | 'cpf' | 'cnpj' | 'date' | 'number' | 'currency' | 'select' | 'textarea'
  placeholder?: string
  required?: boolean
  options?: string[]
}
...
export const FIELDS_BY_TYPE: Record<DocumentTypeId, DocumentField[]> = {
  'cpr-fisica': [
    { name: 'emitente_nome', label: 'Nome do Emitente', type: 'text', required: true },
    { name: 'emitente_cpf_cnpj', label: 'CPF/CNPJ do Emitente', type: 'text', required: true },
    { name: 'emitente_endereco', label: 'Endereço do Emitente', type: 'text', required: true },
    { name: 'credor_nome', label: 'Nome do Credor', type: 'text', required: true },
    { name: 'credor_cpf_cnpj', label: 'CPF/CNPJ do Credor', type: 'text', required: true },
    { name: 'produto', label: 'Produto', type: 'select', options: ['Soja', 'Milho', 'Café', 'Trigo', 'Algodão'], required: true },
    { name: 'quantidade', label: 'Quantidade (sacas)', type: 'number', required: true },
  ...
  'cpr-financeira': [
    ...
    { name: 'credor_nome', label: 'Nome do Credor', type: 'text', required: true },
    { name: 'credor_cpf_cnpj', label: 'CPF/CNPJ do Credor', type: 'text', required: true },
    { name: 'produto', label: 'Produto de Referência', type: 'select', options: ['Soja', 'Milho', 'Café', 'Trigo', 'Algodão'], required: true },
    { name: 'valor_total', label: 'Valor Total (R$)', type: 'currency', required: true },
    { name: 'data_emissao', label: 'Data de Emissão', type: 'date', required: true },
    { name: 'data_vencimento', label: 'Data de Vencimento', type: 'date', required: true },
    { name: 'indice_correcao', label: 'Índice de Correção', type: 'select', options: ['IPCA', 'IGP-M', 'Nenhum'] },
  ],
  'contrato-compra-venda': [
    { name: 'vendedor_nome', label: 'Nome do Vendedor', type: 'text', required: true },
    { name: 'vendedor_cpf_cnpj', label: 'CPF/CNPJ do Vendedor', type: 'text', required: true },
    { name: 'comprador_nome', label: 'Nome do Comprador', type: 'text', required: true },
    { name: 'comprador_cpf_cnpj', label: 'CPF/CNPJ do Comprador', type: 'text', required: true },
...
export const OPTIONAL_CLAUSES: OptionalClause[] = [
  {
    ...
    description: 'Permite ao credor exigir pagamento imediato em caso de inadimplência.',
    appliesTo: [],
  ...
  },
  {
    ...
    description: 'Atualização do valor pelo Índice de Preços ao Consumidor Amplo.',
  ...
  },
  {
    ...
    description: 'Obrigação de contratação de seguro para a produção vinculada.',
  ...
  },
  {
    ...
    description: 'Obrigação de registro do título em cartório de imóveis ou títulos.',
