...
import { ArrowLeft, Clock, MessageSquare, MoreVertical, LayoutGrid, Calendar } from 'lucide-react'
    ...
    if (error || !project) {
        return (
            <div className="flex h-full flex-col items-center justify-center gap-4 p-8 text-center">
                ...
                <h2 className="font-display text-xl font-semibold text-verity-900">Projeto não encontrado</h2>
                <p className="text-sm text-verity-600">O projeto que você procura não existe ou foi removido.</p>
        ...
        <div className="flex flex-1 flex-col overflow-y-auto bg-sand-100 p-6 md:p-8">
            ...
            >
                <Button
                    ...
                    size="sm"
                    onClick={() => router.back()}
                    className="mb-4 -ml-2 text-verity-600 hover:text-verity-900"
                >
                ...
                <div className="flex items-start justify-between">
                    <div>
                        <h1 className="font-display text-3xl font-bold text-verity-900">{project.title}</h1>
                        {project.description && (
                            <p className="mt-2 text-lg text-verity-600">{project.description}</p>
                        )}
            ...
            >
                <h2 className="mb-4 font-display text-xl font-semibold text-verity-900">Conversas do Projeto</h2>
                ...
                {sessions.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-sand-400 bg-sand-50 p-12 text-center">
                        ...
                        <h3 className="mb-1 font-medium text-verity-900">Nenhuma conversa ainda</h3>
                        <p className="mb-4 text-sm text-verity-500">Inicie uma nova análise associada a este projeto.</p>
                        <Button onClick={() => router.push(`/chat?project_id=${project.id}`)}>
                            Nova Conversa
                        </Button>
                    </div>
                ) : (
