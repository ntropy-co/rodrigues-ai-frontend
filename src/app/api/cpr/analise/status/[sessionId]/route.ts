/**
 * BFF (Next.js API Route) — CPR Analysis Status Polling
 *
 * Polls workflow status for active CPR analysis session.
 *
 * Frontend:
 * - `GET /api/cpr/analise/status/[sessionId]`
 *
 * Backend:
 * - `GET ${BACKEND_URL}/api/v1/cpr/analise/status/{sessionId}`
 *
 * Auth:
 * - HttpOnly cookie: verity_access_token (sent automatically)
 *
 * Response:
 * - text: string (last message from assistant)
 * - session_id: string
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/api/v1/cpr/analise/status/${sessionId}`,
      {
        method: 'GET',
        headers: {
          Cookie: cookieHeader,
    ...
    if (!response.ok) {
      ...
      console.error('[API /api/cpr/analise/status] Backend error:', errorDetail)
      return NextResponse.json(
        { detail: errorDetail },
    ...
    console.error('[API /api/cpr/analise/status] Error:', error)
    return NextResponse.json(
