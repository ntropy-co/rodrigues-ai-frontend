...
import { ComplianceVerifier } from '@/features/compliance'
import {
  RiskCalculator,
              ...
              {index < WORKFLOW_STEPS.length - 1 && (
                <div className="mx-1 h-0.5 flex-1">
                  <motion.div
                    ...
                    style={{ maxWidth: '100%' }}
                  />
                  <div className="-mt-0.5 h-0.5 w-full bg-sand-300" />
                </div>
              )}
            </div>
          )
    ...
    >
      ...
      {error && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mx-4 mt-4 flex items-center gap-2 rounded-lg border border-error-200 bg-error-50 p-3"
        >
          <AlertCircle className="h-5 w-5 shrink-0 text-error-600" />
          <p className="flex-1 text-sm text-error-700">{error}</p>
          <button
            onClick={() => reset()}
