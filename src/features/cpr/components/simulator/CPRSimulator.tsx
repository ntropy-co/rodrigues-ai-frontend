...
export function CPRSimulator() {
  ...
  const { data: marketData, isFetching: loadingQuote } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `/api/quotes?symbol=${encodeURIComponent(commodity)}`
      )
  ...
  const currentResult = useMemo(() => {
    ...
    if (!qty || !prc || !term || isNaN(rt) || qty <= 0 || prc <= 0 || term <= 0)
      return null
    ...
    <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <Card className="lg:col-span-2">
        <CardHeader>
          ...
          <CardDescription>
            Calcule o valor líquido da CPR Financeira descontada a valor
            presente.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select
                value={commodity}
                onValueChange={(v) => setCommodity(v as CommoditySymbol)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                {price && marketData?.price && (
                  <div className="pointer-events-none absolute right-2 top-1/2 flex -translate-y-1/2 items-center gap-1 rounded bg-muted px-2 py-1 text-xs">
                    {loadingQuote ? (
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {history.length > 0 && (
              ...
              >
                <Trash2Icon className="h-4 w-4 text-muted-foreground hover:text-error-500" />
              </Button>
            )}
          </div>
        </CardHeader>
            ...
            <div className="space-y-3">
              {history.map((item) => (
                <div
                  ...
                  className="group cursor-pointer rounded-md border p-3 text-sm transition-colors hover:bg-muted/50"
                >
