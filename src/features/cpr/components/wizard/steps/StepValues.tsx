      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            className={errors.amount ? 'border-error-600' : ''}
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            value={data.quantity || ''}
            onChange={(e) =>
              updateData({ quantity: parseFloat(e.target.value) })
            }
            className={errors.quantity ? 'border-error-500' : ''}
          />
          {errors.quantity && (
            <span className="text-xs text-error-500">{errors.quantity}</span>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) =>
              updateData({ unitPrice: parseFloat(e.target.value) })
            }
          />
          <span className="text-xs text-muted-foreground">
            Calculado automaticamente (pode ajustar)
          </span>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(val) =>
              updateData({
                correctionIndex: val as CPRWizardData['correctionIndex']
              })
            }
          >
            <SelectTrigger
              id="correction-index"
              className={errors.correctionIndex ? 'border-error-600' : ''}
              aria-invalid={!!errors.correctionIndex}
              aria-describedby={
                errors.correctionIndex ? 'correction-error' : undefined
              }
            >
        ...
        <div className="space-y-2">
          ...
          />
          {errors.issueDate && (
            <span className="text-xs text-error-500">{errors.issueDate}</span>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.dueDate && (
            <span className="text-xs text-error-500">{errors.dueDate}</span>
          )}
        </div>
