...
export const stepPropriedadeSchema = z.object({
  ...
  farmCar: z
    .string()
    .regex(
      /^[A-Z]{2}-\d{7}-[A-Z0-9]{32}$|^.{15,}$/,
      'CAR inválido ou incompleto'
    )
    .optional()
    .or(z.literal('')),
  farmArea: z.number().positive('Área deve ser positiva'),
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="col-span-2 space-y-2">
          ...
          <Input
            ...
            className={errors.farmName ? 'border-error-500' : ''}
          />
          {errors.farmName && (
            <span className="text-xs text-error-500">{errors.farmName}</span>
          )}
        </div>
        ...
        <div className="col-span-2 space-y-2">
          ...
          <Input
            ...
            value={data.farmCar || ''}
            onChange={(e) =>
              updateData({ farmCar: e.target.value.toUpperCase() })
            }
            className={errors.farmCar ? 'border-error-500' : ''}
          />
          {errors.farmCar && (
            <span className="text-xs text-error-500">{errors.farmCar}</span>
          )}
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            value={data.farmArea || ''}
            onChange={(e) =>
              updateData({ farmArea: parseFloat(e.target.value) })
            }
            className={errors.farmArea ? 'border-error-500' : ''}
          />
          {errors.farmArea && (
            <span className="text-xs text-error-500">{errors.farmArea}</span>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            <SelectTrigger
              id="farmState"
              className={errors.farmState ? 'border-error-500' : ''}
            >
          ...
          {errors.farmState && (
            <span className="text-xs text-error-500">{errors.farmState}</span>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            className={errors.farmCity ? 'border-error-500' : ''}
          />
          {errors.farmCity && (
            <span className="text-xs text-error-500">{errors.farmCity}</span>
          )}
        </div>
        ...
        <div className="col-span-2 space-y-2">
          ...
          <Input
            ...
            className={errors.farmAddress ? 'border-error-500' : ''}
          />
          {errors.farmAddress && (
            <span className="text-xs text-error-500">{errors.farmAddress}</span>
          )}
        </div>
      </div>
