...
> = {
  low: { label: 'Baixo', color: 'text-verity-600', bgColor: 'bg-verity-500' },
  medium: {
    label: 'Médio',
    color: 'text-ouro-600',
    bgColor: 'bg-ouro-500'
  },
  high: { label: 'Alto', color: 'text-ouro-600', bgColor: 'bg-ouro-500' },
  critical: {
    label: 'Crítico',
...
function FactorList({ factors, type }: FactorListProps) {
  ...
  const iconColor = type === 'positive' ? 'text-verity-500' : 'text-error-500'
