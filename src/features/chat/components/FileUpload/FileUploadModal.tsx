...
export function FileUploadModal({
  ...
  const getFileIcon = () => {
    if (files.length === 0)
      return <Upload className="text-gemini-verity-300 h-12 w-12" />
    if (success) return <CheckCircle className="h-12 w-12 text-verity-500" />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div
          className={`relative rounded-xl border-2 border-dashed p-8 text-center transition-colors ${
            dragActive
              ...
              : 'border-gemini-sand-400 bg-gemini-sand-50'
          }`}
          ...
          {files.length > 0 ? (
            <div className="space-y-2">
              {mode === 'attach' && files.length > 1 ? (
                ...
                <div className="space-y-2">
                  <p className="text-gemini-verity-900 font-medium">
                  ...
                  <div className="max-h-40 space-y-1 overflow-y-auto">
                    {files.map((file, index) => (
                      <div
                        key={index}
                        className="bg-gemini-sand-200 flex items-center justify-between rounded px-2 py-1"
                      >
                        ...
                        <button
                          ...
                          className="text-gemini-sand-500 ml-2 hover:text-error-500"
                ...
                <>
                  ...
                  {uploading && (
                    <div className="mt-4">
                      <div className="bg-gemini-sand-300 h-2 w-full rounded-full">
                        <div
                          className="bg-gemini-blue h-2 rounded-full transition-all"
                          style={{ width: `${uploadProgress}%` }}
                        />
                      </div>
                      <p className="text-gemini-verity-500 mt-2 text-sm">
                        {success
            ...
            <div className="space-y-2">
              <p className="text-gemini-verity-600">
              ...
              <p className="text-gemini-sand-500 text-sm">
        ...
        <div className="mt-6 flex gap-3">
          <button
            onClick={handleClose}
            className="flex-1 rounded-lg border border-sand-400 px-4 py-2 text-verity-600 transition-colors hover:bg-sand-50"
            disabled={uploading}
          >
            Cancelar
          </button>
          <button
            ...
            className={`flex-1 rounded-lg px-4 py-2 font-medium text-white transition-colors ${
              files.length === 0 || uploading || success
                ? 'cursor-not-allowed bg-sand-400'
